...
# We can define functions using the `def` keyword
def multiply(x, y):
    # the 'return' keyword allows us to use the result of this calculation
    return x * y

# functions can call themselves recursively! (up to a limit)
def factorial(n):
    if (n == 0):
        return 1
    elif n > 0:
...
# notice that some of this function's arguments have default values. we
# can call this function without specifying these arguments.
def count(stop, start=0, how_long=False):
   ...
   """
   # if the first expression in a function, class, or module is a string, it is called
   # a 'docstring,' and serves as documentation. The compiler stores this in the __doc__
   # attribute of the object (used by the help() function)
   t = time.clock()
   i = start
   while i < stop:
       print(i)
...
# we'll just be iterating over an empty iterable (so the inner block never executes!)
